...
link = 'https://wazirx.com/exchange/BTC-INR'
...
def get_info(currency_list):
    try:
        currency_table = driver.find_element_by_xpath('/html/body/div/div/div[2]/div[2]/div[1]/div/div[2]')
        currencies = currency_table.find_elements_by_css_selector('.ticker-item')
        count = 0
        info = []
        for currency in currencies:
            name = currency.find_element_by_css_selector('.market').find_element_by_css_selector('.market-name-text').text.split('/')[0]
            price = currency.find_element_by_css_selector('.price').text[1:]
            price = 'Rs' + price
            difference = currency.find_element_by_css_selector('.market').find_element_by_css_selector('.market-change').text.split(' ')[1]
...
while True:
    ...
    if scraped_info is not None and inr_others is not None:
        ...
        for i in range(0, len(df.index)):
            pp = json.dumps({'CryptoName': df['Currency'][i], 'Website': 'WazirX',
                             'Values': [['WazirX', usd_df['Price'][i], usd_df['Price'][i], usd_df['Difference'][i]],
                                        ['WazirX', gbp_df['Price'][i], gbp_df['Price'][i], gbp_df['Difference'][i]],
                                        ['WazirX', eur_df['Price'][i], eur_df['Price'][i], eur_df['Difference'][i]],
                                        ['WazirX', cny_df['Price'][i], cny_df['Price'][i], cny_df['Difference'][i]],
                                        ['WazirX', df['Price'][i], df['Price'][i], df['Difference'][i]],
                                        ['WazirX', jpy_df['Price'][i], jpy_df['Price'][i], jpy_df['Difference'][i]]]})
            print(pp)
            try:
                ws.send(pp)
            except:
